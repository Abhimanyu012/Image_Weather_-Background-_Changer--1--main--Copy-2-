...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const historyFilterControls = document.getElementById(
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          ...
          originalImage.onload = () => {
            ...
            document
              .querySelector(".pixelation-controls")
  ...
  // Handle pixelation (now weather transform)
  if (pixelateBtn) {
    pixelateBtn.addEventListener("click", async () => {
      ...
      // Get selected weather type from UI (e.g., from a dropdown or style selector)
      let weatherType = "rain"; // Default, replace with actual UI selection logic
      const styleSelector = document.querySelector(".current-style");
      if (styleSelector && styleSelector.textContent) {
        weatherType = styleSelector.textContent.trim().toLowerCase();
      }
      await applyWeatherTransform(originalImage, weatherType);
    });
  }
...
function closePopup(popup, overlay) {
  ...
  setTimeout(() => {
    ...
    const visiblePopups = document.querySelectorAll(
      ".aspect-ratio-selector.show, .style-selector.show, .limit-modal"
    );
    const anyVisible = Array.from(visiblePopups).some(
      (el) =>
...
function saveImageToHistory(imageUrl, prompt) {
  ...
  const historyItem = {
    ...
    pixelatedImage: imageUrl, // ensure history display works
    prompt: prompt,
...
function addToSessionHistory(item) {
  ...
  // Add style and ratio if available from global state
  if (!item.style && typeof currentStyle !== 'undefined') item.style = currentStyle;
  if (!item.ratio && typeof currentRatio !== 'undefined') item.ratio = currentRatio;
      ...
      historyItem.innerHTML = `
             <div class="history-image-container">
                 <div class="history-image-wrapper">
                     <img src="${item.pixelatedImage}" alt="Pixelated image ${
        index + 1
      }" class="history-preview-image"
                         data-src="${item.pixelatedImage}"
                         data-orig-src="${item.pixelatedImage}">
                     <div class="history-actions">
                         <button class="btn btn-icon history-favorite" data-id="${
                           item.id
                         }" title="Add to favorites">
                             <i class="far fa-star"></i>
                         </button>
                         <button class="btn btn-icon history-download" data-url="${
                           item.pixelatedImage
                         }" title="Download">
                             <i class="fas fa-download"></i>
                         </button>
                         <button class="btn btn-icon history-delete" data-id="${
                           item.id
                         }" title="Remove">
             ...
             <div class="history-info">
                 <p class="history-prompt" title="${item.prompt}">${
        item.prompt
      }</p>
                 <div class="history-meta">
                     <span class="history-date" title="${new Date(
                       item.timestamp
                     ).toLocaleString()}">
...
function applyFilters() {
  ...
  if (styleFilter !== "all") {
    filteredHistory = filteredHistory.filter(
      (item) => (item.style || "Smart") === styleFilter
    );
  }
  ...
  if (filteredItems.length === 0) {
    ...
    filteredItems.forEach((item, index) => {
      ...
      historyItem.innerHTML = `
             <div class="history-image-container">
                 <div class="history-image-wrapper">
                     <img src="${item.pixelatedImage || item.url}" alt="${
        item.prompt
      }" class="history-preview-image"
  ...
  modal.innerHTML = `
     <div class="modal-content1">
         ...
         <div class="modal-body1">
             <img src="${item.url}" data-src="${item.url}" data-orig-src="${
    item.url
  }" alt="${item.prompt}">
         </div>
...
function closeRatioSelector(ratioPopup, overlay) {
  ...
  setTimeout(() => {
    ...
    if (
      (!stylePopup || !stylePopup.classList.contains("show")) &&
...
function closeStyleSelector(stylePopup, overlay) {
  ...
  setTimeout(() => {
    ...
    if (
      (!ratioPopup || !ratioPopup.classList.contains("show")) &&
      (!limitModal || limitModal.style.display !== "block")
    ) {
      overlay.style.display = "none";
    }
...
function updateSelectedRatio() {
  ...
  const selectedOption = document.querySelector(
    `.ratio-option[data-ratio="${currentRatio}"]`
  );
  if (selectedOption) {
...
function updateSelectedStyle() {
  ...
  const selectedOption = document.querySelector(
    `.style-option[data-style="${currentStyle}"]`
  );
  if (selectedOption) {
    selectedOption.classList.add("selected");
  }
...
async function handleImageGeneration(promptInput, resultSection, generatedImg) {
  ...
  try {
    ...
    if (imageUrl) {
      ...
      generatedImg.setAttribute("data-src", imageUrl);
      generatedImg.setAttribute("data-orig-src", imageUrl);
    ...
    if (errorMessageElement) {
      if (error.message.includes("timed out")) {
        errorMessageElement.textContent =
          "The request timed out. Please try again with a different prompt.";
...
class AnimationController {
          ...
          if (matchingItem.isGroup) {
            ...
            elements.forEach((element, elementIndex) => {
              this.animateElement(element, {
                ...
                delay:
                  matchingItem.delay +
                  elementIndex * (matchingItem.stagger || 100),
                duration:
                  matchingItem.duration ||
                  this.config.animationDefaults.duration,
                easing:
                  matchingItem.easing || this.config.animationDefaults.easing,
              });
            ...
            this.animateElement(entry.target, {
              ...
              duration:
                matchingItem.duration || this.config.animationDefaults.duration,
              easing:
                matchingItem.easing || this.config.animationDefaults.easing,
            });
  ...
  findElementSection(element) {
    for (const [sectionName, sequence] of Object.entries(
  ...
  prepareElements() {
    ...
    Object.values(this.config.sequences)
      .flat()
      .forEach((item) => {
        const elements = document.querySelectorAll(item.selector);
          ...
          elements.forEach((element) => {
            if (
              ...
              this.animateElement(element, {
                ...
                delay,
                duration:
                  item.duration || this.config.animationDefaults.duration,
                easing: item.easing || this.config.animationDefaults.easing,
              });
              this.animatedElements.add(element);
            }
...
// --- Weather Transform API Integration ---
let lastWeatherEffect = null;
let lastOriginalImage = null;

async function applyWeatherTransform(image, weatherType) {
  try {
    ...
    // Prepare API request
    const apiUrl =
      "https://4kdc9rr2a2.execute-api.us-east-1.amazonaws.com/default/imageGenerationEndpoints";
    const body = JSON.stringify({
      function: "weather-transform",
      image: base64Image,
      prompt: weatherType,
    });
    ...
    if (data.status === "success" && data.cloudinaryUrl) {
      // Show result
      const pixelatedImg = document.getElementById("pixelatedImg");
      pixelatedImg.src = data.cloudinaryUrl;
      pixelatedImg.setAttribute("data-src", data.cloudinaryUrl);
      pixelatedImg.setAttribute("data-orig-src", data.cloudinaryUrl);
      pixelatedImg.onload = () => {
        ...
        addToSessionHistory({
          ...
          pixelatedImage: data.cloudinaryUrl,
          weatherType: weatherType,
          timestamp: new Date().toISOString(),
        });
        // Store last effect and image for regenerate
        lastWeatherEffect = weatherType;
      ...
      throw new Error(data.message || "Weather transform failed");
    }
  } catch (error) {
    console.error("Error processing weather transform:", error);
    showError(
      "Failed to apply weather effect. Please try again. Error: " +
        error.message
    );
    document.querySelector(".loading-spinner").style.display = "none";
...
if (regenerateBtn) {
  regenerateBtn.addEventListener("click", async function () {
    if (lastOriginalImage && lastWeatherEffect) {
      await applyWeatherTransform(lastOriginalImage, lastWeatherEffect);
    } else {
      showError("No previous weather effect to re-apply.");
    }
  });
}
